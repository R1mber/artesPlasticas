...
export default function Draw({route, navigation}) {
  const {id, tema} = route.params;
  const {nombre, Descripcion, imagen, parrafos, puntaje, tiempo, autor, fecha, tarea, video} = tema.data();
  ...
  const requestPermission = async () => {
    try {
      const granted = await PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.READ_EXTERNAL_STORAGE, {
        ...
        message: 'Esta aplicación necesita acceso al almacenamiento externo para funcionar correctamente',
  ...
  const videoSelect = () => {
    if (video === '3') {
      return '../assets/video3.mp4';
    } else if (video === '4') {
      return '../assets/video4.mp4';
    } else if (video === '5') {
      return '../assets/video5.mp4';
    }
  };
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.body}>
        ...
        <View style={styles.inline}>
          <TouchableOpacity onPress={uploadFile} style={styles.btnCargar}>
            <Text style={styles.textWhite}>Seleccionar archivo</Text>
          </TouchableOpacity>
